...
def supabase_insert(table, data):
    ...
    try:
        ...
        print(f"HTTPError on insert to {table}: {e.code} - {body}")
        raise e
...
def main():
    print("=== STEP 1: RESTORE MISSING DRINKS FROM CSV FILES ===")
    cocktails = supabase_get('cocktails')
    cocktails_zh = supabase_get('cocktailsZH')

    print(f"Initial cocktails: {len(cocktails)}, cocktailsZH: {len(cocktails_zh)}")

    menu_lookup = {}
    for item in MENU_ITEMS:
        norm_en = normalize_name(item['name'])
        menu_lookup[norm_en] = item
        if 'name_zh' in item:
            norm_zh = normalize_name(item['name_zh'])
            menu_lookup[norm_zh] = item
    ...
    for csv_r in csv_recipes:
        ...
        if norm not in existing_en_names:
            ...
            print(f"Restored missing CSV drink: {c_name} (ID: {new_id[:8]}...)")

    print(f"Restored {inserted_csv_count} missing CSV drinks into 'cocktails'.")

    # Fetch fresh copies
    cocktails = supabase_get('cocktails')
    cocktails_zh = supabase_get('cocktailsZH')

    print("\n=== STEP 2: BIDIRECTIONAL SYNC & 1:1 ID ALIGNMENT ===")

    zh_by_id = {r['id']: r for r in cocktails_zh}
    en_by_id = {r['id']: r for r in cocktails}

    # Map cocktailsZH by title to find matching mismatched ID rows
    zh_by_title = {}
    for r in cocktails_zh:
        zh_by_title[normalize_name(r['name'])] = r

    # 2A. Align cocktailsZH with cocktails (1:1 IDs)
    for c_row in cocktails:
        ...
        if c_id not in zh_by_id:
            # Check if there is an existing cocktailsZH row matching by title
            title_zh = menu_match.get('name_zh') or translate_safe(c_name, 'zh')
            matched_zh = zh_by_title.get(normalize_name(title_zh)) or zh_by_title.get(norm_name)

            if matched_zh:
                # Delete old mismatched ID row and insert with c_id
                supabase_delete('cocktailsZH', matched_zh['id'])
                payload_zh = {
                    'id': c_id,
                    'name': matched_zh['name'] or title_zh,
                    'category': c_row.get('category') or matched_zh.get('category'),
                    'glass': matched_zh.get('glass') or c_row.get('glass'),
    ...
    # Re-fetch cocktailsZH and cocktails to verify
    cocktails = supabase_get('cocktails')
    cocktails_zh = supabase_get('cocktailsZH')

    zh_ids = set(r['id'] for r in cocktails_zh)
    en_ids = set(r['id'] for r in cocktails)

    # 2B. Reverse Sync: any cocktailsZH row missing in cocktails
    for zh_row in cocktails_zh:
        zh_id = zh_row['id']
        zh_name = zh_row['name']

        if zh_id not in en_ids:
            # Need to create matching record in cocktails with zh_id
            cat = zh_row.get('category', 'Stirred')
            if cat not in ['Stirred', 'Shaken', 'Bomb', 'Shot']:
                cat = 'Stirred'
    ...
    print(f"Final 'cocktails' count: {len(final_en)}")
    print(f"Final 'cocktailsZH' count: {len(final_zh)}")
